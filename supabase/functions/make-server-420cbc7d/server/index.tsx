...
// ── Client Logos ─────────────────────────────────────────────────────────────
const LOGOS_PREFIX = 'client_logos_420cbc7d:';

api.get('/logos', async (c) => {
  try {
    const logos = await kv.getByPrefix(LOGOS_PREFIX);
    const sorted = logos
      .filter((l: any) => l != null)
...
api.post('/logos', async (c) => {
  ...
  try {
    const body = await c.req.json();
    const now = new Date().toISOString();
    const id = body.id || `logo-${Date.now()}`;
    const logo = { id, name: body.name || '', description: body.description || '', logoUrl: body.logoUrl || '', createdAt: now };
    await kv.set(`${LOGOS_PREFIX}${id}`, logo);
